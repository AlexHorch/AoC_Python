...
text = """dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
xuabbxdwkutpsogcfea[tgetfqpgstsxrokcemk]cbftstsldgcqbxf[vwjejomptmifhdulc]ejeroshnazbwjjzofbe""".split("\n")


def tls(line):
    matches = []
    for i in range(len(line)-3):
        if line[i] is line[i+3] and line[i+1] is line[i+2] and not line[i] is line[i+1]:
            matches.append(i)
    braces = []
    idx = 0
    while(True):
        if not "[" in line[idx:]:
            break
        start = line[idx:].index("[")+idx
        end = line[idx:].index("]")+idx
        braces.append([start, end])
        idx = end+1
    for m in matches:
        for p in braces:
            if m in range(p[0], p[1]):
                return False
...
def ssl(line):
    ...
    while(idx < len(line)):
        if "[" in line[idx:]:
            s = line[idx:].index("[")+idx
            e = line[idx:].index("]")+idx
            outside.append(line[idx: s])
            inside.append(line[s+1:e])
            idx = e+1
        else:
            outside.append(line[idx:])
            idx = len(line)
    combos = []
    for piece in outside:
        for i in range(len(piece)-2):
            if piece[i] is piece[i+2] and piece[i+1] is not piece[i]:
                combos.append(piece[i:i+2])
    for piece in inside:
        for i in range(len(piece)-2):
            if piece[i] is piece[i+2] and piece[i+1] is not piece[i]:
                if piece[i:i+2][::-1] in combos:
                    return True
    return False


print(len(list(filter(lambda x: x, map(lambda line: ssl(line), text)))))

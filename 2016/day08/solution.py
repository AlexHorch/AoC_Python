screen = [["."]*50, ["."]*50, ["."]*50, ["."]*50, ["."]*50, ["."]*50]


def rect(line):
    (a, b) = map(int, line[4:].split("x"))
    for i in range(a):
        for j in range(b):
            screen[j][i] = "#"
...
def row(line):
    ...
    old = [screen[a][i] for i in range(50)]
    for i in range(50):
        screen[a][(i+b) % 50] = old[i]


def column(line):
    (a, b) = map(int, line[16:].split(" by "))
    old = [screen[i][a] for i in range(6)]
    for i in range(6):
        screen[(i+b) % 6][a] = old[i]
...
[print("".join(screen[i])) for i in range(6)]

total = 0
for row in screen:
    total += sum(map(lambda x: 1 if x == "#" else 0,row))
print(total)
